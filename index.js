...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: '⌛ Calma, se esta procesando ⌛',
				success: '✔️ Listo ✔️',
                                levelon: '❬ ✔ ❭ *habilitar Nivel*',
				leveloff: ' ❬ X ❭  *desabilitar Nivel*',
				levelnoton: '❬ X ❭ *Nivel no activo*',
				levelnol: '*Nivel* 0 ',
				error: {
				...
				only: {
					...
					ownerG: '[❗] Este comando solo puede ser usado por el propietario! ❌',
					ownerB: '[❗] Este comando sólo puede ser utilizado por 🐍🐉 𝕾𝖆𝖙𝖆𝖓 🎈🐤! ❌',
					...
					Badmin: '[❗] Este comando sólo se puede utilizar cuando el bot se convierte en administrador! ❌',
                                        daftarB: `── 「REGISTRE-SE」 ──\nHola como\no está registrado en la base de datos, \n\nuse comando : ${prefix}daftar Nombre|edad\nEjemplo : ${prefix}daftar 🐍🐉 𝕾𝖆𝖙𝖆𝖓 🎈🐤|18`,
				}
			}
			...
			const ownerNumber = ["50373488366@s.whatsapp.net"] // replace this with your number
			...
			const isOwner = ownerNumber.includes(sender)
                        ...
                        const NomerOwner = '50373488366@s.whatsapp.net'
                        /******ApiKey Input******/
...
case 'timer':
				...
				case 'demote':
					...
					if (mentioned.length > 1) {
						...
						for (let _ of mentioned) {
							teks += `Solicitud recibida, eliminada de la posición de administrador :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					} else {
						mentions(`Solicitud recibida, eliminada de la posición de administrador @${mentioned[0].split('@')[0]}\n*${groupMetadata.subject}*_`, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					}
					break
                                case 'randomhentai':
                  ...
                  case 'promote':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('Etiqueta a quien desea promover!')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'Solicitud recibida, añadiendo posición como administrador :\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						mentions(teks, mentioned, true)
						...
						mentions(`Solicitud recibida, añadiendo posición como administrador : @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					}
					break
				  case 'wa.me':
  ...
  client.updatePresence(from, Presence.composing) 
      options = {
          text: `「 *SELF WHATSAPP* 」\n\n_Solicitado por_ : *@${sender.split("@s.whatsapp.net")[0]}\n\nSu link Whatsapp : *https://wa.me/${sender.split("@s.whatsapp.net")[0]}*\n*o ( / )*\n*https://api.whatsapp.com/send?phone=${sender.split("@")[0]}*`,
    ...
    client.sendMessage(from, options, text, { quoted: mek } )
			...
			case 'quotes':
              ...
              case 'owner':
                case 'creator':
                  client.sendMessage(from, {displayname: "🐍🐉 𝕾𝖆𝖙𝖆𝖓 🎈🐤", vcard: vcard}, MessageType.contact, { quoted: mek})
               client.sendMessage(from, 'Aquí está el número de mi dueño.',MessageType.text, { quoted: mek} )
                break
	case 'hidetag':
                client.updatePresence(from, Presence.composing) 
                if (!isUser) return reply(mess.only.daftarB)
              ...
              await client.sendMessage(from, options, text)
               break
				...
				case 'snowwrite':
					...
					var gbl7 = gh.split("|")[0];
					var gbl8 = gh.split("|")[1];
					if (args.length < 1) return reply(`Enviar pedidos ${prefix}snowwrite texto1|texto2, ejemplo ${prefix}snowwrite Satan|BOT`)
                                        if (!isUser) return reply(mess.only.daftarB)
					reply(mess.wait)
				...
				case 'marvellogo':
					...
					if (args.length < 1) return reply(`Enviar pedidos ${prefix}marvellogo texto, por ejemplo ${prefix}marvellogo Satan BOT`)
                                        if (!isUser) return reply(mess.only.daftarB)
					reply(mess.wait)
		...
		case 'infonomor':
               client.updatePresence(from, Presence.composing) 
                 if (!isUser) return reply(mess.only.daftarB)
                 if (args.length < 1) return reply(`Introducir números\nEjemplo : ${prefix}información del número 556299663...`)
		...
		case 'spamcall':
               client.updatePresence(from, Presence.composing)
                 if (!isUser) return reply(mess.only.daftarB)
                 if (args.length < 1) return reply(`Introducir numeros\nEjemplo : ${prefix}spamcall 556299663...`)
                ...
                break
                   ...
                   break
				case 'wiki':
					if (args.length < 1) return reply('Digite palabras clave')
					tels = body.slice(6)	
                                        if (!isUser) return reply(mess.only.daftarB)				
				...
				case 'ytmp4':
					...
					if (anu.error) return reply(anu.error)
					teks = `*❏ Title* : ${anu.title}\n\n*VÍDEO QUE SE ESTÁ PREPARANDO, NO SPAM YA QUERIDO*`
					thumb = await getBuffer(anu.thumb)
					client.sendMessage(from, thumb, image, {quoted: mek, caption: teks})
					buffer = await getBuffer(anu.result)
				...
				case 'testime':
					setTimeout( () => {
					client.sendMessage(from, 'Timeout:v', text) // ur cods
					}, 10000) // 1000 = 1s,
					setTimeout( () => {
					client.sendMessage(from, 'Quedan 5 segundos', text) // ur cods
					}, 5000) // 1000 = 1s,
					setTimeout( () => {
				...
				case 'nulis': 
				case 'tulis':
					if (args.length < 1) return reply('Te dije que escribieras, eeeeeeeeeh?')
                                        if (!isUser) return reply(mess.only.daftarB)
					teks = body.slice(7)
					reply(mess.wait)
	...
	case 'kbbi':
            client.updatePresence(from, Presence.composing) 
                if (args.length < 1) return reply(`Digite las perguntas\nEjemplo : ${prefix}hi kbbi`)
	      tels = body.slice(6)
			...
			tels = body.slice(6)
               ...
               if (data.error) return reply(data.error)
				...
				case 'tts':
				   ...
				   if (args.length < 1) return client.sendMessage(from, '¿Cuál es el código del idioma?', text, {quoted: mek})
                                   if (!isUser) return reply(mess.only.daftarB)
					...
					rano = getRandom('.ogg')
					dtt.length > 600
					? reply('El texto es en su mayoría om')
				...
				case 'block':
					...
					client.sendMessage(from, `Pedido recevidos, bloquear a ${body.slice(8)} @c.us`, text)
					break
				case 'hilih':
					client.updatePresence(from, Presence.composing) 
				...
				client.updatePresence(from, Presence.composing) 
					...
					mentions('╔══✪〘 Mencionando Todos 〙✪══\n╠➥'+teks+'╚═〘 🐍🐉  Ş卂𝕥αή  💚♨ BOT 〙', members_id, true)
					break
                case 'tagall2':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
				...
				client.updatePresence(from, Presence.composing) 
					...
					client.sendMessage(from, '╔══✪〘 Mencionando Todos 〙✪══\n╠➥'+teks+'╚═〘 🐍🐉  Ş卂𝕥αή  💚♨ BOT 〙', text, {detectLinks: false, quoted: mek})
					break
                        case 'tagall4':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
				...
				case 'setppbot':
				client.updatePresence(from, Presence.composing) 
				if (!isQuotedImage) return reply(`Envie foto para colocar ${prefix}setbotpp o etiquetas de imagen que ya se han enviado`)
					if (!isOwner) return reply(mess.only.ownerB)
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						reply('Grupo de difusión exitoso')
					}
					break
				case 'alay':
                    client.updatePresence(from, Presence.composing) 
					...
					client.groupLeave (from) 
                     ...
                     setTimeout( () => {
					client.updatePresence(from, Presence.composing) 
					client.sendMessage(from, 'Sayonara pedazo de aborto, te falto odio👋', text) // ur cods
					}, 0)
                     break

				case 'chord':
					if (args.length < 1) return reply('¿dónde está el título de la canción')
                                        if (!isUser) return reply(mess.only.daftarB)
					tels = body.slice(7)					
           ...
           break
				...
				case 'add':
					...
					try {
						...
						console.log('Error :', e)
						reply('No se pudo agregar ala persona, tal vez porque es privado')
					}
					break
				...
				case 'kick':
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'Pedidos recevidos, emitidos :\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						mentions(teks, mentioned, true)
						...
						mentions(`Pedidos recevidos, emitidos : @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupRemove(from, mentioned)
				...
				case 'ocr':
					...
					break
                     ...
                     const bug = body.slice(5)
                      if (pesan.length > 300) return client.sendMessage(from, 'Lo sentimos, el texto es demasiado largo, máximo 300 letras', msgType.text, {quoted: mek})
                    ...
                    reply('Los problemas han sido reportados al propietario del bot, los informes falsos no serán respondidos.')
                    break
               case 'apakah':
               client.updatePresence(from, Presence.composing) 
	    ...
	    case 'kapankah':
			...
			case 'closegc':
				...
				case 'toimg':
				    client.updatePresence(from, Presence.composing)
                                    if (!isUser) return reply(mess.only.daftarB)
					if (!isQuotedSticker) return reply('❌ Sólo pegatinas ❌')
					reply(mess.wait)
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						fs.unlinkSync(media)
						if (err) return reply('❌ No se pudo convertir pegatinas en imágenes ❌')
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						fs.unlinkSync(media)
						if (err) return reply('❌ No se pudo convertir vídeo a mp3 ❌')
						buffer = fs.readFileSync(ran)
               ...
               if (anu.error) return reply(anu.error)
                ...
                break
                   ...
                   break
					...
					break
                                  case 'daftar':
					client.updatePresence(from, Presence.composing)
					if (isUser) return reply('ya estás registrado')
					if (args.length < 1) return reply(`Parámetro incorrecto \nCommand : ${prefix}daftar nmbre|idade\nEjemplo : ${prefix}daftar Satan|18`)
					var reg = body.slice(8)
					var jeneng = reg.split("|")[0];
					var umure = reg.split("|")[1];
						user.push(sender)
						fs.writeFileSync('./database/json/user.json', JSON.stringify(user))
						client.sendMessage(from, `\`\`\`La inscripción fue exitosa con SN: TM08GK8PPHBSJDH10J\`\`\`\n\n\`\`\`En ${date} ${time}\`\`\`\n\`\`\`[Nombre]: ${jeneng}\`\`\`\n\`\`\`[Número]: wa.me/${sender.split("@")[0]}\`\`\`\n\`\`\`[Edad]: ${umure}\`\`\`\n\`\`\`Para usar o bot\`\`\`\n\`\`\`Por favor\`\`\`\n\`\`\`enviar ${prefix}help\`\`\`\n\`\`\`\nTotal de usuários ${user.length}\`\`\``, text, {quoted: mek})
					break
                                case 'welcome':
					if (!isGroup) return reply(mess.only.group)
                                        if (!isUser) return reply(mess.only.daftarB)
				...
				if (isUser) return reply(mess.only.daftarB)
					...
					break
                                case 'nsfwneko':
				    try{
						if (!isNsfw) return reply('❌ *NSFW AHORA ATIVADO* ❌')
                                                if (!isUser) return reply(mess.only.daftarB)
						res = await fetchJson(`https://tobz-api.herokuapp.com/api/nsfwneko?apikey=BotWeA`, {method: 'get'})
						buffer = await getBuffer(res.result)
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'repugnante'})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply('❌ *ERROR* ❌')
					}
				...
				case 'logowolf':
					...
					if (args.length < 1) return reply(`Donde esta el texto? ejemplo ${prefix}logowolf Satan|BOT`)
					reply(mess.wait)
					...
					if (args.length < 1) return reply('digite 1 para ativar')
					if (Number(args[0]) === 1) {
						if (isNsfw) return reply('recurso está ativo')
						nsfw.push(from)
						fs.writeFileSync('./database/json/nsfw.json', JSON.stringify(nsfw))
						reply('❬ BIEN ❭ activó la característica nsfw en este grupo')
					} else if (Number(args[0]) === 0) {
						nsfw.splice(from, 1)
						fs.writeFileSync('./database/json/nsfw.json', JSON.stringify(nsfw))
						reply('❬ SUCESSO ❭ desativado o recurso nsfw neste grupo')
					} else {
						reply('digite 1 para ativar, 0 para desactivar o recurso')
					}
					break	
				case 'bucin':
				...
				case 'logowolf2':
					...
					var teks1 = gh.split("|")[0];
					var teks2 = gh.split("|")[1];
					if (args.length < 1) return reply(`Donde está el texto? exemplo ${prefix}logowolf Toin|BOT`)
                                        if (!isUser) return reply(mess.only.daftarB)
					reply(mess.wait)
            ...
            case 'leveling':
                if (!isGroup) return reply(mess.only.group)
                if (!isGroupAdmins) return reply(mess.only.admin)
                if (args.length < 1) return reply('Digite 1 para activar o recurso')
                if (args[0] === '1') {
                    if (isLevelingOn) return reply('*la función de nivel ya estaba activa antes*')
                    ...
                    reply(` *Digite el comando 1 para activar, 0 para desactivar *\n * Ejemplo: ${prefix}leveling 1*`)
                }
            break
                                case 'infogempa':
				...
				if (!isUser) return reply(mess.only.daftarB)
			       ...
			       reply(anu.result.jadwalTV)
					...
					break
				case 'ttp':
					if (args.length < 1) return reply('Envía un mensaje de texto')
					ranp = getRandom('.png')
					rano = getRandom('.webp')
				...
				case 'clone':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (args.length < 1) return reply('Tag a quien clonara')
					...
					try {
						...
						mentions(`Foto de perfil Se actualizó correctamente usando foto de perfil @${id.split('@')[0]}`, [jid], true)
					} catch (e) {
						reply('Fallo la clonacion')
					}
					break
                                /*case 'magernulis':
                                        var gh = body.slice(11)
